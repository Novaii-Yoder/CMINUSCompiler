/*  Symbol Table --linked list
    ...
    Modified 4/14/2023
    Jacob Yoder
    added check_params function to see if actual and formal paramaters match.
    
    changed Display() and PrintSym() so that it also shows type of symbol when printing.
*/
...
// POST: if they are the same length and type
// 1 means equal, 0 means not same length and type
int check_params(ASTnode *a, ASTnode *f){
	if(a == NULL && f == NULL) {
		...
		//printf("IN RECERSIVE CALL\n");
		return check_params(a->next, f->next);
	}
	return 0;
} // end of check_params
